...
geometry_msgs::Vector3 outdoor_gps;
...
void poseCallback(const sensor_msgs::NavSatFix::ConstPtr& msg)
{
   ...
   //ROS_INFO("Delta_long: %f",delta_long);
	...
	//거리계산
	//distance = sqrt(pow(outdoor_gps_utm_currentPos.x - initial_pos_utm.x,2.0)+pow(outdoor_gps_utm_currentPos.y-initial_pos_utm.y,2.0));
...
void distanceCallback(const geometry_msgs::PoseStamped& msg)
{
	outdoor_gps_utm_currentPos.x = msg.pose.position.x;
	outdoor_gps_utm_currentPos.y = msg.pose.position.y;
...
int main(int argc, char **argv)
{
  ...
  roll = pitch = yaw = 0.0;  
    ros::Subscriber subOGps = n.subscribe("/ublox_gps/fix",10, &poseCallback);
    ros::Subscriber subutm = n.subscribe("/utm",10, &distanceCallback);
  ...
  ROS_INFO("Initial Pos UTM %d N %.7lf %.7lf",utm_zone,initial_pos_utm.x,initial_pos_utm.y);
   //ROS_INFO("Outdoor GPS lat : %.7lf   long : %.7lf UTM %d N %.7lf %.7lf", outdoor_gps_currentPos.x, outdoor_gps_currentPos.y,utm_zone,outdoor_gps_utm_currentPos.x,outdoor_gps_utm_currentPos.y); 
    ROS_INFO("Outdoor GPS UTM %d N %.7lf %.7lf", utm_zone,outdoor_gps_utm_currentPos.x,outdoor_gps_utm_currentPos.y); 
