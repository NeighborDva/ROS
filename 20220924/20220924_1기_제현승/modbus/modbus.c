...
void check_serial_data(uint8_t read_buf[])
{
	crc_16_modbus.data = CRC16_MODBUS(read_buf, 4);
	...
	printf("MODBUS Result CRC: 0x%02x 0x%02x \n\n", crc_16_modbus.bytedata[0], crc_16_modbus.bytedata[1]);
	
	if(crc_16_modbus.bytedata[0] == read_buf[4] && crc_16_modbus.bytedata[1] == read_buf[5])
	{
		printf("There are no errors in the received data.\n");
	}
	else
	{
		printf("There are errors in the received data.\n");
	}
}
...

{
	...
	for(i = 0 ; i < 4 ; i++)
	{	
		printf("%d.HEX Num : ",i+1);
		scanf("%x", &hex_num);
		while(getchar() != '\n'); //줄바꿈문자 제거
		read_buf[i] = hex_num;
	}
	
	 m_crc.data =  CRC16_MODBUS(read_buf,4);
	 read_buf[4] = m_crc.bytedata[0];
	 read_buf[5] = m_crc.bytedata[1];
