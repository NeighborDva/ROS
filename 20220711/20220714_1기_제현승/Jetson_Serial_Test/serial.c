...

{
...

    {
...

        {
			...
			read_buf[g_ucCmdATCount] = read_serial_data ;
					...
					{
						if(i<2){
						I_origin.bytedata[i] = read_buf[g_ucCmdATCount];
						i++;
						}
						else if(i<4)
						 read_buf[g_ucCmdATCount]= 0;
						 else
						 i = 0;
					...
					{
						if(i<4){
						F_origin.bytedata[i] = read_buf[g_ucCmdATCount];
						i++;
						}
						else 
						i=0;
			...
			read_buf[g_ucCmdATCount] = read_serial_data ;
					...
					crc.bytedata[i] = read_buf[g_ucCmdATCount];
					g_ucCmdATCount++ ;
    ...
    int crc_check = crc_cal();//crc 검증;
    
    if(crc_check == 0)
		printf("There are no errors in the data.");
	else
		printf("An error was detected in the data.");
