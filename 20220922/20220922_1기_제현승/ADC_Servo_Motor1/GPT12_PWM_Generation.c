...
#define PWM_FREQUENCY               50.0f                    /* Frequency of the PWM signal in Hz -->서보모터 주파수 50Hz 20ms의 주기를 가져야 하기때문                    */
#define PWM_DUTY_CYCLE              7.25                      /* Duty cycle of the PWM signal in percentage-- > 7.25 ->90도           */
...
void initGpt12PWM(void)
{
    ...
    test_value = fTimer;
    float32 dutyUpTime = (fTimer * (PWM_DUTY_CYCLE / 100.0f)) / PWM_FREQUENCY;
    float32 dutyDownTime = (fTimer * (1 - (PWM_DUTY_CYCLE / 100.0f))) / PWM_FREQUENCY;
    ...
    IfxPort_setPinModeOutput(TESTPIN1, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);
}
...
float32 map_result_ang = 0.0;
float32 map_servo(int x, int in_min, int in_max, float out_min, float out_max)
{
    map_result_ang = (float32)((x - in_min) * (out_max - out_min)) / (in_max - in_min) + out_min;
    return  map_result_ang;
}
float32 pwm_duty = 0.0;
void Set_angle(uint8 angle)
{
    pwm_duty = map_servo(angle, 0, 180, 2.5, 12.0);
    float32 dutyUpTime = (fTimer * (pwm_duty / 100.0f)) / PWM_FREQUENCY;
    float32 dutyDownTime = (fTimer * (1-(pwm_duty / 100.0f))) / PWM_FREQUENCY;
    IfxGpt12_T2_setTimerValue(&MODULE_GPT120, dutyDownTime);
    IfxGpt12_T4_setTimerValue(&MODULE_GPT120, dutyUpTime);
}
